...
func (m *defaultUserLegalizeLogModel) FindOne(ctx context.Context, legalizeId int64) (*UserLegalizeLog, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, userLegalizeLogLegalizeIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
func (m *defaultUserLegalizeLogModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheUserLegalizeLogLegalizeIdPrefix, primary)
}

func (m *defaultUserLegalizeLogModel) queryPrimary(ctx context.Context, conn sqlx.SqlConn, v, primary interface{}) error {
	query := fmt.Sprintf("select %s from %s where `legalize_id` = ? limit 1", userLegalizeLogRows, m.table)
