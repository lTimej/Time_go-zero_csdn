...
func (m *defaultUserQualificationModel) FindOne(ctx context.Context, qualificationId int64) (*UserQualification, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, userQualificationQualificationIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
func (m *defaultUserQualificationModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheUserQualificationQualificationIdPrefix, primary)
}

func (m *defaultUserQualificationModel) queryPrimary(ctx context.Context, conn sqlx.SqlConn, v, primary interface{}) error {
	query := fmt.Sprintf("select %s from %s where `qualification_id` = ? limit 1", userQualificationRows, m.table)
